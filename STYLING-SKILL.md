...
:root {
  ...
  /* Letter wave (experimental, opt in with blyrics-letter-wave) */
...
| `blyrics-letter-wave` | `false` | Experimental. Split words into per-letter spans and float each letter as it is sung, over the word wobble. Tune with `--blyrics-letter-wave-*`; long words also get a per-letter scale swell |
...
Float each letter of a word in turn as it is sung, on top of the word wobble:
...
It layers on the word wobble rather than replacing it, so the default `scaleX` pop stays. To make the letters carry all the motion instead, set the word wobble to identity (`--blyrics-word-wobble-transform-*: translateY(0)`). It follows `--blyrics-animate-word-wobble`, so reduced motion turns it off. The split multiplies the DOM per character and reruns the karaoke sweep per letter, so keep it to themes that want the effect.
