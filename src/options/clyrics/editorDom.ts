...
export const contextMenus: { [key: string]: ContextData[] } = {
    ...
    word: [ // pair this with the context menu `line` 
...
export function addNewLine(data: Lyric) {
    ...
    const wordParts = data.parts && data.parts.length > 0 ? data.parts : [{
    ...
    //// SVG
    switch (data.agent) {
        case "v1": normalLine.innerHTML = domDefaults.svg.leftAlign; break;
        case "v2": normalLine.innerHTML = domDefaults.svg.rightAlign; break;
        case "v3": normalLine.innerHTML = domDefaults.svg.rightAlign; break;
        case "v1000": normalLine.innerHTML = domDefaults.svg.middleAlign; break;
        default: normalLine.innerHTML = domDefaults.svg.justify; break;
    }
    ...
    wordParts.filter(val => val.isBackground && val.words && val.words.length < 1).forEach(part => {
        const partWord = part.words;
        const allSpaces = partWord.trim().length < 1;
