...
function initNicknameModal(): void {
  ...
  const NICKNAME_STATUS_ICONS: Record<string, string> = {
  ...
  const NICKNAME_STATUS_ICON_FOR: Record<NicknameStatusKind, keyof typeof NICKNAME_STATUS_ICONS | null> = {
    idle: null,
    typing: null,
    checking: "spinner",
    available: "check",
  ...
  const setStatus = (kind: NicknameStatusKind): void => {
    ...
    const iconHtml = iconKey ? NICKNAME_STATUS_ICONS[iconKey] : "";
    const text = t(`options_nickname_status_${kind}`);
    status.innerHTML = `${iconHtml}<span>${text}</span>`;
  };
